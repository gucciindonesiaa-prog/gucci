...
export const AuthPage: React.FC<AuthPageProps> = ({ onSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isRegister) {
      ...
      // Save user info including password for verification
      localStorage.setItem('gucci_user_id', formData.userId);
      localStorage.setItem('gucci_user_phone', formData.phone);
      localStorage.setItem('gucci_user_password', formData.password);
      ...
      const storedPassword = localStorage.getItem('gucci_user_password');
      
      if (!storedUserId) {
        setError("Akun belum terdaftar di perangkat ini. Wajib daftar Akun Kerja terlebih dahulu.");
        return;
      }

      if (storedUserId !== formData.userId) {
        setError("ID Kerja tidak ditemukan atau tidak cocok dengan data pendaftaran.");
        return;
      }
